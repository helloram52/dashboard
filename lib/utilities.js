...
var buildCustomerObjectFn = function(customerRevenueMap, customerId, revenue, Country, prodMix) {
	var revenueSum = 0, prodMixRevenue = 0;
	...
	if(customerRevenueMap.hasOwnProperty(customerId)) {
		...
		if(customerRevenueMap[customerId].hasOwnProperty(Country)){
			...
			if(customerRevenueMap[customerId][Country].hasOwnProperty(prodMix)){
				...
				prodMixRevenue = customerRevenueMap[customerId][Country][prodMix];
				customerRevenueMap[customerId][Country][prodMix] = revenue + prodMixRevenue;
			}
			else{

				customerRevenueMap[customerId][Country][prodMix] = revenue;	
			}
		}
		else {
		...
		if(revenueMaps.hasOwnProperty(busUnit)) {
			...
			if(revenueMaps[busUnit].hasOwnProperty(year)) {
				...
				if(revenueMaps[busUnit][year].hasOwnProperty(month)) {
					...
					this.buildCustomerObject(customerRevenueMap,customerId, revenue, Country, prodMix);
			...
			revenueMaps[busUnit][year][month][customerId][Country][prodMix] = revenue;
		}
	}
